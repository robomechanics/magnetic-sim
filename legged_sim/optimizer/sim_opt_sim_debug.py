...
# ── Debug state ───────────────────────────────────────────────────────────────
_dbg_step     = 0
_DBG_INTERVAL = 200   # print every N physics steps (~0.1 s at dt=0.0005)
...
def _apply_mag(model, data, sphere_gids, plate_ids, magnet_ids, params, off_mids=None):
    ...
    _fromto  = np.zeros(6)
    do_print = (_dbg_step % _DBG_INTERVAL == 0)
    _dbg_step += 1
    ...
    for mid in magnet_ids:
        ...
        for gid in sphere_gids[mid]:
            ...
            if best_dist < min_dist_seen:
                min_dist_seen = best_dist

            if best_dist <= 0:
                n_zero_dist += 1
                continue
            if best_dist > params['max_magnetic_distance']:
                n_beyond_max += 1
                continue
        ...
        if do_print:
            mid_name = mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_BODY, mid) or f"body{mid}"
            fmag_out = np.linalg.norm(data.xfrc_applied[mid, :3])
            reasons  = []
            if n_zero_dist  > 0: reasons.append(f"PENETRATION({n_zero_dist}/{n_spheres})")
            if n_beyond_max > 0: reasons.append(f"BEYOND_MAX_DIST({n_beyond_max}/{n_spheres})")
            if clamped:          reasons.append(f"FORCE_CLAMPED(raw={total_mag:.2f}N > max={params['max_force_per_wheel']:.2f}N)")
            print(
                f"[t={data.time:.3f}] {mid_name:30s} | "
                f"min_dist={min_dist_seen*1000:+7.3f}mm | "
                f"contributing={n_contributing}/{n_spheres} | "
                f"F_out={fmag_out:.2f}N | "
...
def run_headless_lift(params: dict) -> tuple[float, float, float]:
    ...
    Phases executed:
      1. Settle  (0 → SETTLE_TIME): all magnets ON, PID hold — sampled from t=0
    ...
    Returns:
        (mean_abs_x, mean_abs_y, mean_abs_z) where:
          XY — mean absolute lateral drift of FR/BL/BR electromagnets,
               sampled over the FULL simulation [0, total_time] (settle
               phase sampled immediately; lift phase after LIFT_MEASURE_START
               to skip the foot-travel transient). Baseline is t=0 position.
          Z  — mean absolute Z deviation of FR/BL/BR electromagnets over
               the same full window, relative to t=0 position. Captures
               both magnet-induced penetration during settle AND load
               redistribution when FL lifts off.
    """
    ...
    # ── Record baselines at t=0 before any forces act ─────────────────────────
    # Drift is now measured across the full simulation (settle + lift),
    # so the baseline is the initial resting position of each stance EE.
    ...
    while data.time < total_time:
        ...
        if t < SETTLE_TIME:
            ...
            # ── Bounce detector ───────────────────────────────────────────────
            if _dbg_step % _DBG_INTERVAL == 0:
                for foot, bid in stance_ee_bids.items():
                    z  = data.xpos[bid][2]
                    vz = data.cvel[bid, 5]
                    print(f"  [SETTLE] t={data.time:.3f}  {foot} ee  z={z*1000:.3f}mm  vz={vz*1000:.3f}mm/s")

            # Sample settle drift — measured from t=0 baseline with no transient skip
            # (robot is stationary during settle so no foot-travel artifact)
            z_drifts = np.array([
        ...
        # ── One-time snapshot at end of settle (IK setup only) ────────────────
        # Baselines are already set from t=0; this block only initialises IK
        # for the lift phase.
        if not settled:
            settled = True
        ...
        # Z: deviation from t=0 baseline — captures both settle-induced
        #    penetration change and lift-induced load redistribution.
        z_drifts = np.array([
            abs(data.xpos[bid][2] - z_settle_baseline[foot])
            for foot, bid in stance_ee_bids.items()
        ])
        ...
        # XY: lateral drift from t=0 baseline — captures cumulative slip
        #     across both settle and lift phases.
        xy_drifts = np.array([
            np.abs(data.xpos[bid][:2] - xy_lift_baseline[foot])
            for foot, bid in stance_ee_bids.items()
        ])
