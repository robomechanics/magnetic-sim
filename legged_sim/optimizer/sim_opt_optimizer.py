"""
sim_optimizer.py — CMA-ES optimizer for the MWC lift sim.

Tunes 14 physics parameters to minimize body XYZ drift during FL lift hold.
...
Usage:
    ...
    python sim_optimizer.py --n-calls 500 --suffix my_run
    python sim_optimizer.py --resume-from results/20250101T120000_my_run
    python sim_optimizer.py --warm-start-from results/20250101T120000_my_run
"""
...
def _evaluate_one_candidate(args):
    """Run a single lift sim trial for one candidate parameter point.

    Args:
        args: tuple of (point_index, point)
    ...
    from sim_opt_sim import run_headless_lift
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CMA-ES sim lift parameter optimization")
    ...
    print(f"\nSim lift optimizer: {n_calls} evals, batch={BATCH_SIZE}")
    print(f"Cost: 50% Z-drift + 25% X-drift + 25% Y-drift (body during lift hold)")
