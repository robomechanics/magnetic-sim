"""
sim_vertopt_config.py — configuration for the vertical wall-hold optimizer.

Cost (lower = better):
  50% — mean absolute X drift (penetration into / pull-off from wall)
  25% — mean absolute Z drift (gravity-direction slip)
...
def calculate_cost(mean_abs_x: float, mean_abs_y: float, mean_abs_z: float) -> dict:
    """
    Weighted EE drift cost during vertical wall hold.
      50% X — penetration into / pull-off from wall
      50% Z — gravity-direction slip (robot dropping)
       0% Y — lateral slip ignored
    """
    x_cost = 0.50 * mean_abs_x
    y_cost = 0.00 * mean_abs_y
