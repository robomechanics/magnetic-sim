...
def setup_model():
    ...
    # Make all robot geoms semi-transparent (skip floor/wall)
...
def main():
    ...
    LIFT_HOLD = 3.0        # seconds to hold lifted pose before hip turn
    HIP_TURN_DUR = 1.5     # seconds to complete the hip rotation
    HIP_TARGET = np.radians(-45.0)  # CW = negative Z rotation

    def hip_trajectory(t_rel, duration, q_start, q_end):
        """Quintic smooth-step interpolation from q_start to q_end over duration."""
        s = np.clip(t_rel / duration, 0.0, 1.0)
        s = s**3 * (6*s**2 - 15*s + 10)
        return q_start + s * (q_end - q_start)

    hip_fl_jid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "hip_pitch_FL")
    hip_turned = [False]
    lifted_home = [None]
    hip_pivot   = [None]
    ...
    def sim_step():
        ...
        t = data.time
        elapsed_since_settle = t - SETTLE_TIME

        data.xfrc_applied[:] = 0
        if args.magnets and settled:
            # After settle: swing foot magnet off, stance feet on.
            off = {swing_mag_bid}
            if t - last_print >= 0.5:
                status = {
                    mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_BODY, mid): 'OFF' if mid in off else 'ON'
                    for mid in magnet_ids
                }
                print(f"  magnets: {status}")
        ...
        if args.no_ik:
            ...
            # --- Compute target_pos for this step across all phases ---

            # Phase 2: hold lifted
            target_pos = ee_home + np.array([0.0, 0.0, 0.10])

            # Phase 3: CW 45 deg — rotate foot around hip pivot via IK
            if elapsed_since_settle >= LIFT_HOLD:
                if not hip_turned[0]:
                    lifted_home[0] = ik.ee_pos(data, SWING_FOOT).copy()
                    hip_pivot[0]   = data.xanchor[hip_fl_jid].copy()
                    hip_turned[0]  = True
                t_cw = elapsed_since_settle - LIFT_HOLD
                angle = hip_trajectory(t_cw, HIP_TURN_DUR, 0.0, HIP_TARGET)
                r = lifted_home[0] - hip_pivot[0]
                c, s = np.cos(angle), np.sin(angle)
                Rz = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])
                target_pos = hip_pivot[0] + Rz @ r

            # Phase 4: CCW 45 deg back — reverse rotation via IK
            CCW_START = LIFT_HOLD + HIP_TURN_DUR
            if elapsed_since_settle >= CCW_START:
                t_ccw = elapsed_since_settle - CCW_START
                angle = hip_trajectory(t_ccw, HIP_TURN_DUR, HIP_TARGET, 0.0)
                r = lifted_home[0] - hip_pivot[0]
                c, s = np.cos(angle), np.sin(angle)
                Rz = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])
                target_pos = hip_pivot[0] + Rz @ r

            # Phase 5: lower back to ground
            LOWER_START = LIFT_HOLD + 2 * HIP_TURN_DUR
            LOWER_DUR = 1.5
            if elapsed_since_settle >= LOWER_START:
                t_lower = elapsed_since_settle - LOWER_START
                dz = hip_trajectory(t_lower, LOWER_DUR, 0.10, 0.0)
                target_pos = ee_home + np.array([0.0, 0.0, dz])

            # --- IK solve with final target_pos ---
            ik_step_counter[0] += 1
    ...
    def draw_target_marker(viewer):
        ...
        # Magnet state arrows: green = on, red = off
        MAG_ARROW_LEN = 0.05
        MAG_ARROW_RAD = 0.005
        up = np.array([0, 0, 1])
        rot_up = np.eye(3)  # Z already points up
        for foot in FEET:
            bid = ik.ee_bids[foot]
            pos = data.xpos[bid].copy()
            pos[2] += MAG_ARROW_LEN / 2 + 0.01  # offset above EE
            is_off = settled and foot == SWING_FOOT
            color = [1.0, 0.1, 0.1, 0.9] if is_off else [0.1, 1.0, 0.1, 0.9]
            if scn.ngeom < scn.maxgeom:
                g = scn.geoms[scn.ngeom]
                mujoco.mjv_initGeom(
                    g, mujoco.mjtGeom.mjGEOM_CAPSULE,
                    [MAG_ARROW_RAD, MAG_ARROW_LEN / 2, 0],
                    pos, rot_up.flatten(), color)
                scn.ngeom += 1
