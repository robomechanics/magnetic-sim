...
COST_WEIGHT_DETACH  = 0.50   # penalty for detaching before GOAL
COST_WEIGHT_XY_DRIFT = 0.50  # penalty for XY position change before detachment
...
def calculate_cost(detach_force: float, detach_moment: float, xy_drift: float) -> dict:
    ...
    # No-attachment sentinel: magnet never stuck to the plate at all.
    # detach_force==0 AND detach_moment==0 means apply_mag returned 0 every step
    # (e.g. max_magnetic_distance too small, Br too low, spheres never in range).
    # Cost 0.50 would conflate this with "detached at half goal" — return max penalty instead.
    ...
    DRIFT_REFERENCE_M = 0.01  # 5mm reference — tune this
...
ACTIVE_PRESET = 'hold'  # set to one of: 'hold', 'drive_sideways', 'drive_up'
