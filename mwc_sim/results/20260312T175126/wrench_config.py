...
GOAL_FORCE  = 100.0   # N   — TODO: set to measured pull-off force
GOAL_WRENCH = 5.0     # N·m — TODO: set to measured peel moment

# Cost weights (must sum to 1.0)
COST_WEIGHT_DETACH  = 0.70   # penalty for detaching before GOAL
COST_WEIGHT_XY_DRIFT = 0.30  # penalty for XY position change before detachment

# ── CMA-ES optimizer settings ─────────────────────────────────────────────────
N_CALLS              = 1000   # total candidate evaluations
BATCH_SIZE           = 16     # CMA-ES population size
CMAES_SIGMA0         = 0.3    # initial step size
OPTIMIZER_RANDOM_STATE = 42
PULL_RATE_OPT        = 20.0   # N/s — fixed pull rate used during optimization
...
space: list = [
    ...
    Real(1e-4,  0.1,   "log-uniform", name="max_magnetic_distance"),
    Real(10.0,  500.0, "log-uniform", name="max_force_per_wheel"),
]
...
def calculate_cost(detach_force: float, detach_moment: float, xy_drift: float) -> dict:
    """Compute optimization cost from a single wrench sim trial.

    Cost breakdown (lower = better):
      - 70%: how far detach force/moment fell short of the goal (early detachment penalty)
      - 30%: normalized XY position drift before detachment (sliding penalty)

    Args:
        detach_force:  Peak horizontal force at detachment (N). Used when APPLY_FORCE=True.
        detach_moment: Peak moment at detachment (N·m). Used when APPLY_MOMENT=True.
        xy_drift:      Total XY displacement of magnet COM from settle end to detachment (m).

    Returns:
        dict with keys: total_cost, detach_cost, drift_cost, achieved, goal
    """
    ...
    if APPLY_MOMENT and not APPLY_FORCE:
        ...
        # Both on: use force as primary metric
        # TODO: define combined objective if both toggles are active
        achieved = detach_force
        goal     = GOAL_FORCE

    # Early detachment penalty: normalized shortfall, clipped to [0, 1]
    # If achieved >= goal, this term is 0 (no penalty).
    # If achieved == 0, this term is 1 (maximum penalty).
    shortfall    = max(0.0, goal - achieved) / goal
    detach_cost  = COST_WEIGHT_DETACH * shortfall

    # XY drift penalty: normalized by a reference drift scale.
    # TODO: set DRIFT_REFERENCE_M to a physically meaningful value once you have
    #       baseline measurements (e.g. max acceptable slide before detachment).
    DRIFT_REFERENCE_M = 0.005  # 5mm reference — tune this
